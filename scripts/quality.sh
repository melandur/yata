...
if [[ -n "${YATA_REQUIRE_DEVICE_TESTS:-}" ]]; then
  options+=(--env YATA_REQUIRE_DEVICE_TESTS)
fi
for variable in YATA_QUALITY_TASK YATA_QUALITY_SHARD; do
...
exec "$engine" run "${options[@]}" \
  ...
  --env "YATA_QUALITY_COMMIT=$(git -C "$repository" rev-parse HEAD)" \
  "$image" bash -euc '
    ...
    case "$1" in all|test)
      case "${YATA_QUALITY_TASK:-test}" in
        ...
        shard)
          xvfb-run -a dbus-run-session -- env -u WAYLAND_DISPLAY GDK_BACKEND=x11 \
            ...
            YATA_REQUIRE_DEVICE_TESTS=1 python3 scripts/quality_ci.py run
          ;;
        test)
          xvfb-run -a dbus-run-session -- env -u WAYLAND_DISPLAY GDK_BACKEND=x11 \
            GTK_A11Y=none NO_AT_BRIDGE=1 YATA_REQUIRE_GTK_TESTS=1 \
          ...
          ;;
        *) echo "Invalid YATA_QUALITY_TASK" >&2; exit 2 ;;
      esac
      ;;
    esac
  ' bash "$phase"
