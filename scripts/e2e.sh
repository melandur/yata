...
user_id="$(id -u)"
group_id="$(id -g)"
image="${YATA_E2E_IMAGE:-}"
image_key="$(python3 "$repository/scripts/e2e_bundle.py" image-key)"
bundle=""
if [[ -n "${YATA_E2E_BUNDLE:-}" ]]; then
  if [[ -z "$image" ]]; then
    echo "A CI bundle requires its pinned runtime image (YATA_E2E_IMAGE)." >&2
    exit 1
  fi
  bundle="$(realpath --relative-to="$repository" "$YATA_E2E_BUNDLE")"
  ...
  image_inputs="$("$engine" image inspect --format '{{ index .Config.Labels "org.yata.e2e.inputs" }}' "$image")"
...
exec "$engine" run "${options[@]}" \
  ...
  --env "YATA_E2E_UPDATE_BASELINES=${YATA_E2E_UPDATE_BASELINES:-0}" \
  --env "YATA_E2E_WORKERS=${YATA_E2E_WORKERS:-auto}" \
  --env "YATA_E2E_BUNDLE=${bundle:+/workspace/$bundle}" \
  "$image" bash -euc '
    mkdir -p "$HOME" "$CARGO_HOME"
    if [[ -n "${YATA_E2E_BUNDLE:-}" ]]; then
      export YATA_BINARY="$YATA_E2E_BUNDLE/yata"
      test -x "$YATA_BINARY"
    else
      rustc --version
      cargo build --locked --bin yata
      export YATA_BINARY="$CARGO_TARGET_DIR/debug/yata"
    fi
